...
function selected_gallery_index() {
  ...
  var button = gradioApp().querySelector(
    '[style="display: block;"].tabitem .gallery-item.\\!ring-2'
  );
...
function get_extras_tab_index() {
  ...
  return [
    get_tab_index("mode_extras"),
    get_tab_index("extras_resize_mode"),
    ...args,
  ];
}
...
onUiUpdate(function () {
  ...
  if (!txt2img_textarea) {
    txt2img_textarea = gradioApp().querySelector(
  ...
  if (!img2img_textarea) {
    img2img_textarea = gradioApp().querySelector(
...
function update_token_counter(button_id) {
  if (token_timeout) clearTimeout(token_timeout);
  token_timeout = setTimeout(
