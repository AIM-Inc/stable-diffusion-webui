...
function check_progressbar(
  ...
  if (
    progressbar != null &&
    progressbar != global_progressbars[id_progressbar]
  ) {
    ...
    var mutationObserver = new MutationObserver(function (m) {
      ...
      if (preview != null && gallery != null) {
        ...
        var progressDiv =
          gradioApp().querySelectorAll("#" + id_progressbar_span).length > 0;
        if (!progressDiv) {
          if (skip) {
...
function check_gallery(id_gallery) {
  ...
  if (gallery && galleries[id_gallery] !== gallery) {
    ...
    galleryObservers[id_gallery] = new MutationObserver(function () {
      let galleryButtons = gradioApp().querySelectorAll(
        "#" + id_gallery + " .gallery-item"
      );
      let galleryBtnSelected = gradioApp().querySelector(
        "#" + id_gallery + " .gallery-item.\\!ring-2"
      );

      if (
        prevSelectedIndex !== -1 &&
        galleryButtons.length > prevSelectedIndex &&
        !galleryBtnSelected
      ) {
    ...
    galleryObservers[id_gallery].observe(gallery, {
      childList: true,
      subtree: false,
    });
  }
