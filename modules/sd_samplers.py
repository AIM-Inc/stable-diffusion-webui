...
from modules import prompt_parser, processing, images
...
def setup_img2img_steps(p, steps=None):
    if opts.img2img_fix_steps or steps is not None:
        steps = (
            int((steps or p.steps) / min(p.denoising_strength, 0.999))
...
class VanillaStableDiffusionSampler:
    def __init__(self, constructor, sd_model):
        ...
        self.orig_p_sample_ddim = (
            self.sampler.p_sample_ddim
            if hasattr(self.sampler, "p_sample_ddim")
            else self.sampler.p_sample_plms
        )
        self.mask = None
        self.nmask = None
...
class CFGDenoiser(torch.nn.Module):
    ...
    def forward(self, x, sigma, uncond, cond, cond_scale, image_cond):
        ...
        x_in = torch.cat(
            [torch.stack([x[i] for _ in range(n)]) for i, n in enumerate(repeats)] + [x]
        )
        ...
        sigma_in = torch.cat(
            [torch.stack([sigma[i] for _ in range(n)]) for i, n in enumerate(repeats)]
            + [sigma]
        )
        ...
        if tensor.shape[1] == uncond.shape[1]:
            ...
            if shared.batch_cond_uncond:
                x_out = self.inner_model(
                    x_in,
                    sigma_in,
                    cond={"c_crossattn": [cond_in], "c_concat": [image_cond_in]},
                )
            else:
                ...
                for batch_offset in range(0, x_out.shape[0], batch_size):
                    ...
                    x_out[a:b] = self.inner_model(
                        ...
                        sigma_in[a:b],
                        cond={
                            "c_crossattn": [cond_in[a:b]],
                            "c_concat": [image_cond_in[a:b]],
                        },
                    )
        else:
            x_out = torch.zeros_like(x_in)
            ...
            x_out[-uncond.shape[0] :] = self.inner_model(
                ...
                cond={
                    "c_crossattn": [uncond],
                    "c_concat": [image_cond_in[-uncond.shape[0] :]],
                },
            )
...
class KDiffusionSampler:
    def __init__(self, funcname, sd_model):
        self.model_wrap = k_diffusion.external.CompVisDenoiser(
            sd_model, quantize=shared.opts.enable_quantization
        )
    ...
    def randn_like(self, x):
        noise = (
            self.sampler_noises[self.sampler_noise_index]
            if self.sampler_noises is not None
            and self.sampler_noise_index < len(self.sampler_noises)
            else None
        )
    ...
    def sample_img2img(
        ...
        if p.sampler_noise_scheduler_override:
            ...
            sigmas = k_diffusion.sampling.get_sigmas_karras(
                n=steps, sigma_min=0.1, sigma_max=10, device=shared.device
            )
        else:
