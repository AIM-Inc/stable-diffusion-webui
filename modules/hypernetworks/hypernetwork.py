...
def apply_strength(value=None):
    HypernetworkModule.multiplier = (
        value if value is not None else shared.opts.sd_hypernetwork_strength
    )
...
def train_hypernetwork(
    ...
    for i, entries in pbar:
        ...
        if len(previous_mean_losses) > 1:
            ...
            std = 0
        dataset_loss_info = (
            f"dataset loss:{mean(previous_mean_losses):.3f}"
            + "\u00B1"
            + f"({std / (len(previous_mean_losses) ** 0.5):.3f})"
        )
        pbar.set_description(dataset_loss_info)

        if (
            hypernetwork.step > 0
            and hypernetwork_dir is not None
            and hypernetwork.step % save_hypernetwork_every == 0
        ):
        ...
        textual_inversion.write_loss(
            ...
            hypernetwork.step,
            len(ds),
            {"loss": f"{previous_mean_loss:.7f}", "learn_rate": scheduler.learn_rate},
        )

        if (
            hypernetwork.step > 0
            and images_dir is not None
            and hypernetwork.step % create_image_every == 0
        ):
