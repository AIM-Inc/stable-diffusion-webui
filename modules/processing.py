    ...
    image = Image.fromarray(
        cv2.cvtColor(
            exposure.match_histograms(
                cv2.cvtColor(np.asarray(image), cv2.COLOR_RGB2LAB),
                correction,
                channel_axis=2,
            ),
            cv2.COLOR_LAB2RGB,
...
def create_infotext(
    p, all_prompts, all_seeds, all_subseeds, comments, iteration=0, position_in_batch=0
):
    ...
    generation_params = {
        ...
        "Hypernet": (
            None
            if shared.loaded_hypernetwork is None
            else shared.loaded_hypernetwork.name
        ),
        ...
        "Eta": (
            None
            if p.sampler is None or p.sampler.eta == p.sampler.default_eta
            else p.sampler.eta
        ),
...
def process_images_inner(p: StableDiffusionProcessing) -> Processed:
    ...
    with open(
        os.path.join(shared.script_path, "params.txt"), "w", encoding="utf8"
    ) as file:
        ...
        for n in range(p.n_iter):
            ...
            with devices.autocast():
                uc = prompt_parser.get_learned_conditioning(
                    shared.sd_model, len(prompts) * [p.negative_prompt], p.steps
                )
...
class StableDiffusionProcessingTxt2Img(StableDiffusionProcessing):
    ...
    def init(self, all_prompts, all_seeds, all_subseeds):
        if self.enable_hr:
            ...
            self.extra_generation_params[
                "First pass size"
            ] = f"{self.firstphase_width}x{self.firstphase_height}"
            ...
            if self.firstphase_width == 0 or self.firstphase_height == 0:
                ...
                if width_ratio > height_ratio:
                    ...
                    firstphase_height_truncated = (
                        self.firstphase_width * self.height / self.width
                    )
                else:
                    firstphase_width_truncated = (
                        self.firstphase_height * self.width / self.height
                    )
                    firstphase_height_truncated = self.firstphase_height

            self.truncate_x = (
                int(self.firstphase_width - firstphase_width_truncated) // opt_f
            )
            self.truncate_y = (
                int(self.firstphase_height - firstphase_height_truncated) // opt_f
            )
    ...
    def create_dummy_mask(self, x, width=None, height=None):
        if self.sampler.conditioning_key in {"hybrid", "concat"}:
            ...
            image_conditioning = self.sd_model.get_first_stage_encoding(
                self.sd_model.encode_first_stage(image_conditioning)
            )
            ...
            image_conditioning = torch.zeros(
                x.shape[0], 5, 1, 1, dtype=x.dtype, device=x.device
            )
    ...
    def sample(
        ...
        self.sampler = sd_samplers.create_sampler_with_index(
            sd_samplers.samplers, self.sampler_index, self.sd_model
        )
        ...
        if not self.enable_hr:
            ...
            samples = self.sampler.sample(
                ...
                conditioning,
                unconditional_conditioning,
                image_conditioning=self.create_dummy_mask(x),
            )
            return samples

        x = create_random_tensors(
            [opt_C, self.firstphase_height // opt_f, self.firstphase_width // opt_f],
            seeds=seeds,
            subseeds=subseeds,
            subseed_strength=self.subseed_strength,
            seed_resize_from_h=self.seed_resize_from_h,
        ...
        samples = self.sampler.sample(
            ...
            conditioning,
            unconditional_conditioning,
            image_conditioning=self.create_dummy_mask(
                x, self.firstphase_width, self.firstphase_height
            ),
        )
        ...
        samples = samples[
            :,
            :,
            self.truncate_y // 2 : samples.shape[2] - self.truncate_y // 2,
...
class StableDiffusionProcessingImg2Img(StableDiffusionProcessing):
    ...
    def init(self, all_prompts, all_seeds, all_subseeds):
        self.sampler = sd_samplers.create_sampler_with_index(
            sd_samplers.samplers_for_img2img, self.sampler_index, self.sd_model
        )
        crop_region = None
        ...
        if self.image_mask is not None:
            ...
            if self.mask_blur > 0:
                self.image_mask = self.image_mask.filter(
            ...
            if self.inpaint_full_res:
                ...
                self.image_mask = images.resize_image(
                    self.resize_mode, self.image_mask, self.width, self.height
                )
        ...
        latent_mask = (
            self.latent_mask if self.latent_mask is not None else self.image_mask
        )
        ...
        if self.sampler.conditioning_key in {"hybrid", "concat"}:
            ...
            conditioning_mask = torch.nn.functional.interpolate(
                conditioning_mask, size=self.init_latent.shape[-2:]
            )
            conditioning_mask = conditioning_mask.expand(
                conditioning_image.shape[0], -1, -1, -1
            )
            self.image_conditioning = torch.cat(
                [conditioning_mask, conditioning_image], dim=1
            )
            self.image_conditioning = self.image_conditioning.to(shared.device).type(
                self.sd_model.dtype
            )
        else:
            self.image_conditioning = torch.zeros(
                self.init_latent.shape[0],
                5,
                1,
                1,
