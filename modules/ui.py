...
if cmd_opts.ngrok != None:
    ...
    ngrok.connect(
        cmd_opts.ngrok,
        cmd_opts.port if cmd_opts.port != None else 7860,
...
def save_files(js_data, images, do_make_zip, index):
    ...
    with open(
        os.path.join(opts.outdir_save, "log.csv"), "a", encoding="utf8", newline=""
    ) as file:
    ...
    return (
        gr.File.update(value=fullfns, visible=True),
        "",
        "",
        plaintext_to_html(f"Saved: {filenames[0]}"),
    )
    ...
    def f(*args, extra_outputs_array=extra_outputs, **kwargs):
        ...
        try:
            ...
            if len(argStr) > max_debug_str_len:
                print(
                    f"(Argument list truncated at {max_debug_str_len}/{len(argStr)} characters)",
                    file=sys.stderr,
                )
            ...
            res = extra_outputs_array + [
                f"<div class='error'>{plaintext_to_html(type(e).__name__+': '+str(e))}</div>"
            ]
        ...
        res[
            -1
        ] += f"<div class='performance'><p class='time'>Time taken: <wbr>{elapsed_text}</p>{vram_html}</div>"
...
def check_progress_call(id_part):
    ...
    time_left = calc_time_left(
        progress, 1, " ETA: ", shared.state.time_left_force_display
    )
    if time_left != "":
    ...
    if opts.show_progress_every_n_steps > 0:
        if shared.parallel_processing_allowed:
            ...
            if (
                shared.state.sampling_step - shared.state.current_image_sampling_step
                >= opts.show_progress_every_n_steps
                and shared.state.current_latent is not None
            ):
                if opts.show_progress_grid:
                    shared.state.current_image = (
                        modules.sd_samplers.samples_to_image_grid(
                            shared.state.current_latent
                        )
                    )
                else:
                    shared.state.current_image = modules.sd_samplers.sample_to_image(
                        shared.state.current_latent
                    )
    ...
    return (
        f"<span id='{id_part}_progress_span' style='display: none'>{time.time()}</span><p>{progressbar}</p>",
        preview_visibility,
        image,
        textinfo_result,
    )
...
def roll_artist(prompt):
    ...
    artist = random.choice(
        [x for x in shared.artist_db.artists if x.category in allowed_cats]
    )
...
def apply_styles(prompt, prompt_neg, style1_name, style2_name):
    prompt = shared.prompt_styles.apply_styles_to_prompt(
        prompt, [style1_name, style2_name]
    )
    prompt_neg = shared.prompt_styles.apply_negative_styles_to_prompt(
        prompt_neg, [style1_name, style2_name]
    )

    return [
        gr.Textbox.update(value=prompt),
        gr.Textbox.update(value=prompt_neg),
...
def create_seed_inputs():
    with gr.Row():
        ...
        with gr.Box(elem_id="subseed_show_box"):
            seed_checkbox = gr.Checkbox(
                label="Extra", elem_id="subseed_show", value=False
            )
    ...
    with gr.Row(visible=False) as seed_extra_row_1:
        ...
        subseed_strength = gr.Slider(
            label="Variation strength", value=0.0, minimum=0, maximum=1, step=0.01
        )

    with gr.Row(visible=False) as seed_extra_row_2:
        seed_extras.append(seed_extra_row_2)
        seed_resize_from_w = gr.Slider(
            minimum=0, maximum=2048, step=64, label="Resize seed from width", value=0
        )
        seed_resize_from_h = gr.Slider(
            minimum=0, maximum=2048, step=64, label="Resize seed from height", value=0
        )
...
def update_token_counter(text, steps):
    ...
    tokens, token_count, max_length = max(
        [model_hijack.tokenize(prompt) for prompt in prompts], key=lambda args: args[1]
    )
    ...
    with gr.Row(elem_id="toprow"):
        ...
        with gr.Column(scale=1, elem_id="roll_col"):
            roll = gr.Button(
                value=art_symbol,
                elem_id="roll",
                visible=len(shared.artist_db.artists) > 0,
            )
            ...
            with gr.Row():
                with gr.Column(scale=1, elem_id="style_pos_col"):
                    prompt_style = gr.Dropdown(
                        label="Style 1",
                        elem_id=f"{id_part}_style_index",
                ...
                with gr.Column(scale=1, elem_id="style_neg_col"):
                    prompt_style2 = gr.Dropdown(
                        label="Style 2",
                        elem_id=f"{id_part}_style2_index",
                        choices=[k for k, v in shared.prompt_styles.styles.items()],
                        value=next(iter(shared.prompt_styles.styles.keys())),
                    )
...
def create_ui(wrap_gradio_gpu_call):
    ...
    with gr.Blocks(analytics_enabled=False) as txt2img_interface:
        ...
        with gr.Row().style(equal_height=False):
            with gr.Column(variant="panel"):
                ...
                with gr.Row(equal_height=True):
                    ...
                    batch_size = gr.Slider(
                        minimum=1, maximum=8, step=1, label="Batch size", value=1
                    )
            ...
            with gr.Column(variant="panel"):
                ...
                with gr.Group():
                    ...
                    txt2img_gallery = gr.Gallery(
                        label="Output", show_label=False, elem_id="txt2img_gallery"
                    ).style(grid=4)
            ...
            connect_reuse_seed(
                seed, reuse_seed, generation_info, dummy_component, is_subseed=False
            )
            connect_reuse_seed(
                subseed,
                reuse_subseed,
            ...
            txt2img_args = dict(
                ...
                + custom_inputs,
                outputs=[txt2img_gallery, generation_info, html_info],
                show_progress=False,
            )
            ...
            txt_prompt_img.change(
                ...
                inputs=[txt_prompt_img],
                outputs=[txt2img_prompt, txt_prompt_img],
            )
            ...
            txt2img_paste_fields = [
                ...
                (
                    hr_options,
                    lambda d: gr.Row.update(visible="Denoising strength" in d),
                ),
    ...
    with gr.Blocks(analytics_enabled=False) as img2img_interface:
        ...
        with gr.Row().style(equal_height=False):
                ...
                with gr.Tabs(elem_id="mode_img2img") as tabs_img2img_mode:
                    with gr.TabItem("img2img", id="img2img"):
                        init_img = gr.Image(
                            ...
                            interactive=True,
                            type="pil",
                            tool=cmd_opts.gradio_img2img_tool,
                        ).style(height=480)
                    ...
                    with gr.TabItem("Inpaint", id="inpaint"):
                        ...
                        with gr.Row():
                            inpaint_full_res = gr.Checkbox(
                                label="Inpaint at full resolution", value=False
                            )
            ...
            with gr.Column(variant="panel"):
                ...
                with gr.Group():
                    ...
                    img2img_gallery = gr.Gallery(
                        label="Output", show_label=False, elem_id="img2img_gallery"
                    ).style(grid=4)
            ...
            img2img_prompt_img.change(
                fn=modules.images.image_data,
                inputs=[img2img_prompt_img],
                outputs=[img2img_prompt, img2img_prompt_img],
            )
            ...
            img2img_args = dict(
                ...
                + custom_inputs,
                outputs=[img2img_gallery, generation_info, html_info],
                show_progress=False,
            )
            ...
            style_dropdowns = [
                (txt2img_prompt_style, txt2img_prompt_style2),
                (img2img_prompt_style, img2img_prompt_style2),
            ]
            style_js_funcs = ["update_txt2img_tokens", "update_img2img_tokens"]

            for button, (prompt, negative_prompt) in zip(
                [txt2img_save_style, img2img_save_style], prompts
            ):
                button.click(
                    fn=add_style,
            ...
            for button, (prompt, negative_prompt), (style1, style2), js_func in zip(
                [txt2img_prompt_style_apply, img2img_prompt_style_apply],
                prompts,
                style_dropdowns,
                style_js_funcs,
            ):
    ...
    with gr.Blocks(analytics_enabled=False) as extras_interface:
        with gr.Row().style(equal_height=False):
            with gr.Column(variant="panel"):
                with gr.Tabs(elem_id="mode_extras"):
                    ...
                    with gr.TabItem("Batch from Directory"):
                        ...
                        show_extras_results = gr.Checkbox(
                            label="Show result images", value=True
                        )

                with gr.Tabs(elem_id="extras_resize_mode"):
                    with gr.TabItem("Scale by"):
                        upscaling_resize = gr.Slider(
                            minimum=1.0, maximum=4.0, step=0.05, label="Resize", value=2
                        )
                    with gr.TabItem("Scale to"):
                        with gr.Group():
                            with gr.Row():
                                upscaling_resize_w = gr.Number(
                                    label="Width", value=512, precision=0
                                )
                                upscaling_resize_h = gr.Number(
                                    label="Height", value=512, precision=0
                                )
                            upscaling_crop = gr.Checkbox(
                                label="Crop to fit", value=True
                            )

                with gr.Group():
                    extras_upscaler_1 = gr.Radio(
                        label="Upscaler 1",
                ...
                with gr.Group():
                    gfpgan_visibility = gr.Slider(
                        ...
                        value=0,
                        interactive=modules.gfpgan_model.have_gfpgan,
                    )
                ...
                with gr.Group():
                    codeformer_visibility = gr.Slider(
                        ...
                        value=0,
                        interactive=modules.codeformer_model.have_codeformer,
                    )
                ...
                submit = gr.Button(
                    "Generate", elem_id="extras_generate", variant="primary"
                )
            ...
            with gr.Column(variant="panel"):
                ...
                button_id = (
                    "hidden_element" if shared.cmd_opts.hide_ui_dir_config else ""
                )
                open_extras_folder = gr.Button(
                    "Open output directory", elem_id=button_id
                )
    ...
    with gr.Blocks() as modelmerger_interface:
        with gr.Row().style(equal_height=False):
            with gr.Column(variant="panel"):
                gr.HTML(
                    value="<p>A merger of the two checkpoints will be generated in your <b>checkpoint</b> directory.</p>"
                )
                ...
                modelmerger_merge = gr.Button(
                    elem_id="modelmerger_merge", label="Merge", variant="primary"
                )
    ...
    with gr.Blocks() as train_interface:
            ...
            with gr.Tabs(elem_id="train_tabs"):
                ...
                with gr.Tab(label="Create hypernetwork"):
                    ...
                    new_hypernetwork_activation_func = gr.Dropdown(
                        value="relu",
                        label="Select activation function of hypernetwork",
                        choices=modules.hypernetworks.ui.keys,
                    )
                    new_hypernetwork_initialization_option = gr.Dropdown(
                        value="Normal",
                        label="Select Layer weights initialization. relu-like - Kaiming, sigmoid-like - Xavier is recommended",
                        choices=[
                            "Normal",
                            "KaimingUniform",
                            "KaimingNormal",
                    ...
                    overwrite_old_hypernetwork = gr.Checkbox(
                        value=False, label="Overwrite Old Hypernetwork"
                    )
                    ...
                    with gr.Row():
                        ...
                        with gr.Column():
                            create_hypernetwork = gr.Button(
                                value="Create hypernetwork", variant="primary"
                            )
                ...
                with gr.Tab(label="Train"):
                    ...
                    with gr.Row():
                        ...
                        create_refresh_button(
                            ...
                            lambda: {
                                "choices": sorted(
                                    [x for x in shared.hypernetworks.keys()]
                                )
                            },
                    ...
                    with gr.Row():
                        ...
                        train_hypernetwork = gr.Button(
                            value="Train Hypernetwork", variant="primary"
                        )
                        train_embedding = gr.Button(
                            value="Train Embedding", variant="primary"
                        )
        ...
        train_embedding.click(
            fn=wrap_gradio_gpu_call(
                modules.textual_inversion.ui.train_embedding,
...
""",
    ...
    def run_settings(*args):
        ...
        for key, value, comp in zip(opts.data_labels.keys(), args, components):
            if comp != dummy_component and not opts.same_type(
        ...
        for key, value, comp in zip(opts.data_labels.keys(), args, components):
            ...
            if (
                comp_args
                and isinstance(comp_args, dict)
                and comp_args.get("visible") is False
            ):
                continue
    ...
    with gr.Blocks(analytics_enabled=False) as settings_interface:
        ...
        quicksettings_names = set(
            x for x in quicksettings_names if x != "quicksettings"
        )
        ...
        with gr.Row(elem_id="settings").style(equal_height=False):
            for i, (k, item) in enumerate(opts.data_labels.items()):
                ...
                if previous_section != item.section:
                    if cols_displayed < settings_cols and (
                        items_displayed >= items_per_col or previous_section is None
                    ):
                    ...
                    gr.HTML(
                        elem_id="settings_header_text_{}".format(elem_id),
                        value='<h1 class="gr-button-lg">{}</h1>'.format(text),
                    )
        ...
        with gr.Row():
            ...
            restart_gradio = gr.Button(
                value="Restart Gradio and Refresh components (Custom Scripts, ui.py, js and css only)",
                variant="primary",
            )
        ...
        reload_script_bodies.click(
            fn=reload_scripts, inputs=[], outputs=[], _js="function(){}"
        )
    ...
    with gr.Blocks(css=css, analytics_enabled=False, title="Stable Diffusion") as demo:
        ...
        txt2img_fields = [
            field for field, name in txt2img_paste_fields if name in paste_field_names
        ]
        img2img_fields = [
            field for field, name in img2img_paste_fields if name in paste_field_names
        ]
        ...
        modules.generation_parameters_copypaste.connect_paste(
            txt2img_paste, txt2img_paste_fields + settings_paste_fields, txt2img_prompt
        )
        modules.generation_parameters_copypaste.connect_paste(
            img2img_paste, img2img_paste_fields + settings_paste_fields, img2img_prompt
        )

        modules.generation_parameters_copypaste.connect_paste(
            pnginfo_send_to_txt2img,
            txt2img_paste_fields + settings_paste_fields,
            generation_info,
            "switch_to_txt2img",
        )
        modules.generation_parameters_copypaste.connect_paste(
            pnginfo_send_to_img2img,
            img2img_paste_fields + settings_paste_fields,
            generation_info,
    ...
    def loadsave(path, x):
        def apply_field(obj, field, condition=None, init_field=None):
            ...
            if saved_value is None:
                ...
                print(
                    f'Warning: Bad ui setting value: {key}: {saved_value}; Default value "{getattr(obj, field)}" will be used instead.'
                )
            else:
        ...
        if (
            type(x) in [gr.Slider, gr.Radio, gr.Checkbox, gr.Textbox, gr.Number]
            and x.visible
        ):
        ...
        if type(x) == gr.Dropdown and getattr(x, "save_to_config", False):
            apply_field(
                x, "value", lambda val: val in x.choices, getattr(x, "init_field", None)
            )
            apply_field(x, "visible")
