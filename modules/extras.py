...
def run_extras(
    ...
    for image, image_name in zip(imageArr, imageNameArr):
        ...
        if gfpgan_visibility > 0:
            restored_img = modules.gfpgan_model.gfpgan_fix_faces(
        ...
        if codeformer_visibility > 0:
            restored_img = modules.codeformer_model.codeformer.restore(
                np.array(image, dtype=np.uint8), w=codeformer_weight
            )
            res = Image.fromarray(restored_img)
        ...
        if resize_mode == 1:
            upscaling_resize = max(
                upscaling_resize_w / image.width, upscaling_resize_h / image.height
            )
            crop_info = " (crop)" if upscaling_crop else ""
            info += (
                f"Resize to: {upscaling_resize_w:g}x{upscaling_resize_h:g}{crop_info}\n"
            )
                ...
                if c is None:
                    ...
                    if mode == 1 and crop:
                        ...
                        cropped.paste(
                            c,
                            box=(
                                resize_w // 2 - c.width // 2,
                                resize_h // 2 - c.height // 2,
                            ),
                        )
                        c = cropped
                    cached_images[key] = c
...
def run_modelmerger(
    ...
    filename = (
        primary_model_info.model_name
        + "_"
        + str(round(1 - multiplier, 2))
        + "-"
        + secondary_model_info.model_name
        + "_"
